    ...
    ISplitEventsAndErrors
{
    ...
    function assertVotingPower(uint256 _tokenId, int256 _amountFP, string memory _message) internal view {
        assertVotingPower(_tokenId, block.timestamp, _amountFP, _message);
    }
    ...
    // lock created and current one. See `LinearDecreasingCurve`'s
    ...
    function _deployCurve(
        ...
        return LinearDecreasingCurve(address(impl).deployUUPSProxy(initCalldata));
    }
