...
contract TestCreateLock is IEscrowCurveTokenStorage, EscrowBase {
    ...
    function testCantMintToZeroAddress() public {
        ...
        escrow.createLockFor(1e18, MAX_TIME, address(0));
    }
    ...
    function testCantMintBelowMinDeposit(uint256 _minDeposit) public {
        ...
        vm.startPrank(address(123));
        {
            ...
            escrow.createLock(_minDeposit - 1, MAX_TIME);
        }
        vm.stopPrank();
    ...
    function testFuzz_createLock(uint128 _value, address _depositor, uint32 _time, uint256 _duration) public {
        ...
        uint256 checkpointInterval = clock.checkpointInterval();
        _duration = _duration / checkpointInterval * checkpointInterval;
        vm.assume(_duration >= checkpointInterval);
        ...
        {
            uint256 vp = bias(_value, block.timestamp - virtualStartTime);
        ...
        // Check the lock was created:
        {
            ...
            assertEq(lock.start, virtualStartTime);
        }
    ...
    function testCreateMultipleLocks() public {
        ...
        // create the locks
        {
            vm.startPrank(matt.addr);
            {
                ...
                vm.expectEmit(true, true, true, true);
                emit Deposit(matt.addr, 1, startTime, startTime, MAX_TIME, matt.value, matt.value);
                escrow.createLock(matt.value, MAX_TIME);
            }
            vm.stopPrank();
            ...
            vm.startPrank(shane.addr);
            {
                ...
                vm.expectEmit(true, true, true, true);
                emit Deposit(shane.addr, 2, startTime, startTime, MAX_TIME, shane.value, total);
                escrow.createLock(shane.value, MAX_TIME);
            }
            vm.stopPrank();
        }
    ...
    function testFuzz_createLockFor(uint128 _value) public {
        ...
        vm.expectEmit(true, true, true, true);
        emit Deposit(_who, 1, startTime, startTime, MAX_TIME, _value, _value);
        escrow.createLockFor(_value, MAX_TIME, _who);
        ...
        vm.expectEmit(true, true, true, true);
        emit Deposit(_contract, 2, startTime, startTime, MAX_TIME, _value, 2 * uint(_value));
        escrow.createLockFor(_value, MAX_TIME, _contract);
    }
