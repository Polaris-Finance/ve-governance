...
contract TestMerge_ApproveDelegateAndTransfer is TestMerge_ApproveDelegateBase {
    ...
    function _removeDelegationAndAssert(uint256 _survivingTokenId) internal override {
        ...
        assertEq(ivotesAdapter.getVotes(eve), bias(totalAmount, block.timestamp - checkpointTs));
    ...
    function test_Merge_PartialTransfer_BobRetainsRemainingPower() public {
        ...
        uint256 checkpointTs3 = weekStartTs(block.timestamp);
        createLockAndMoveToNextWeek(amount3, MAX_TIME); // tokenId 3
        ...
        uint256 elapsed3 = block.timestamp - checkpointTs3;
        assertEq(ivotesAdapter.numberOfDelegatedTokens(alice), 1);
        assertEq(ivotesAdapter.numberOfDelegatedTokens(dave), 1);
        assertEq(ivotesAdapter.getVotes(bob), bias(amount3, elapsed3));
    ...
    function test_Merge_PartialTransfer_BobGaugeVoteDecreases() public {
        ...
        token.approve(address(escrow), amount3);
        uint256 checkpointTs3 = weekStartTs(block.timestamp);
        createLockAndMoveToNextWeek(amount3, MAX_TIME);
        ivotesAdapter.delegate(bob);
        vm.stopPrank();
        ...
        uint256 elapsed1 = block.timestamp - checkpointTs;
        uint256 elapsed3 = block.timestamp - checkpointTs3;
        assertEq(voter.votes(bob, gauge), bias(totalAmount, elapsed1) + bias(amount3, elapsed3));
        assertEq(ivotesAdapter.getVotes(bob), bias(totalAmount, elapsed1) + bias(amount3, elapsed3));
        ...
        vm.warp(block.timestamp + 1 weeks);
        elapsed1 = block.timestamp - checkpointTs;
        elapsed3 = block.timestamp - checkpointTs3;

        // Bob's gauge vote auto-decreases without revoting
        // TODO: assertEq(voter.votes(bob, gauge), bias(amount3, elapsed3));
        assertEq(ivotesAdapter.getVotes(bob), bias(amount3, elapsed3));
        assertEq(ivotesAdapter.getVotes(eve), bias(totalAmount, elapsed1));
    }
}
